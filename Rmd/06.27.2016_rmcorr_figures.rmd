...
rtest <- cor.test(raz2005$Age, raz2005$Volume)
print(rtest)
...
text(25,107,paste('rmcorr =', round(brainvolage.rmc$r,2),'\np < 0.001'), adj = 0)
...
axis.break(axis = 2, style = "slash")
text(25,107,paste('r =', round(rtest$est,2),'\np < 0.001'), adj = 0)
...
print(avg.lm)
rtest <- cor.test(gildenMeans$rt, gildenMeans$acc)
print(rtest)
```
...
cols <- my.pal(dim(gildenMeans)[1])

plot(acc~rt, data = gildenMeans, cex = 1.2, pch = 16, col = cols, 
     ylim = c(.79, 1), xlim = c(0.45, .95),
     xlab = "Reaction Time (seconds)",  ylab = "Accuracy", 
     main = ("b) Simple Regression\n(Averaged Data)"))
abline(avg.lm, col = "gray", lwd = 2)
axis.break(axis = 1, style = "slash")
axis.break(axis = 2, style = "slash")
text(.95,.8,paste('r =', round(rtest$est,2),'\np =', round(rtest$p.value,2)), adj = 1)
