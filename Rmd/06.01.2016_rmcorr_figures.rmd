...
par(mfrow = c(1, 2), mgp = c(2.5, .75, 0), mar = c(4,4,2,1))

plot(example.rmc, exampleMat, xlab = "x", ylab = "y", main = "Rmcorr Plot",
     overall = F, palette = mypal, las = 1)
text(-3,-8,paste('rmcorr =', round(example.rmc$r,2),'\np < 0.001'), adj = 0)

plot(submeanx[,2], submeany[,2], pch = 16, col = cols, main = "Regression Plot", 
...
par(mfrow=c(1,3), mar = c(4,4,2,2), mgp = c(2.75, .75, 0))
...
plot(example2.rmc, exampleMat2, xlab = "x", ylab = "", main = "x / 2 + 1", overall = F,
...
plot(example3.rmc, exampleMat3, xlab = "x", ylab = "", main = "Subject 3: y - 2", overall = F,
     palette = mypal, xlim = c(-3.5, 1), ylim = c(-2.25,2), las = 1)
...
makeminiplot <- function(subxs, sub.slope, intercept, constant=0, xax = "n", 
  ...
  if (legend) legend('bottomright', legend = "OLS", lwd = 1, bty = "n",
                     cex = 1.25, inset = .02)
}
...
###11. Multi-level modeling of size and distance data 
...
#Model 4: Model 3 + varying int for actual dist by participant (discontinous for actual dist, allow diff ints by participant)
Model4 <-lmer(Size_distance$Emmert_distance_meters ~ Size_distance$Actual_dist_meters*Size_distance$Perceived_dist +
                (0 + Size_distance$Actual_dist_meters| Size_distance$Participant), 
              REML = FALSE)

#Good reference for distcont change: Singer and Willett (2003), Chapter 6
#R code:http://www.ats.ucla.edu/stat/r/examples/alda/ch6.htm
...
##### b) Graph the best fit model: Model 4 
```{r, echo=FALSE, fig.width = 9, fig.height = 6}
plot(Emmert_distance_meters~Perceived_dist, data = Size_distance, pch = 16, 
     col=rgb(red = 0.0, green = 0.0, blue = 0.0, alpha = 0.3),
...
Predict2<-subset(SizeDistBestFit,Actual_dist_meters == 2)
Predict4<-subset(SizeDistBestFit,Actual_dist_meters == 4)
Predict6<-subset(SizeDistBestFit,Actual_dist_meters == 6)
...
ablineclip(FixedEffectsMod4[1], SlopeFE, col = "red",  x1 = XYRange[1], x2 = XYRange[2])

#Plot Overall fixed effects w/discont
#Actual distance has a mean = 4, sd = 2 
#Ints
Int2<-FixedEffectsMod4[1]+FixedEffectsMod4[2]*(2)
Int4<-FixedEffectsMod4[1]+FixedEffectsMod4[2]*(4)
Int6<-FixedEffectsMod4[1]+FixedEffectsMod4[2]*(6)

#Slopes (sd * mean)
Slope2<-FixedEffectsMod4[3]+FixedEffectsMod4[4]*(-1*2)
Slope4<-FixedEffectsMod4[3]+FixedEffectsMod4[4]*(0*2)
Slope6<-FixedEffectsMod4[3]+FixedEffectsMod4[4]*(1*2)



getranges(Predict2)
ablineclip(Int2, Slope2, col = "red", lty = 2, x1 = XYRange[1], x2 = XYRange[2])

getranges(Predict4)
ablineclip(Int4, Slope2, col = "red", lty = 2, x1 = XYRange[1], x2 = XYRange[2])

getranges(Predict6)
ablineclip(Int6, Slope2, col = "red", lty = 2, x1 = XYRange[1], x2 = XYRange[2])


#Plot random effects
PlotRE<- function(N, length, PredictData) 
{
  for (i in 1:N) 
  {
    Extract<-1+(i-1)*length  #Starts at 1, then goes to 7, 13, ..., 115 
    abline(lines(spline(PredictData[Extract:(length*i), 6],PredictData[Extract:(length*i), 11])), col = "grey", lty = 1, lwd = 1)
  }
}
NumParticipants<-20
PlotRE(NumParticipants, 6, Predict2)
PlotRE(NumParticipants, 6, Predict4)
PlotRE(NumParticipants, 6, Predict6)

dev.copy(pdf, file="plots/Figure11_multilevel_model.pdf", 9, 6)  
dev.off()
```
