...
#### Running Code Below Requires: dplyr, patchwork, and esc   
```{r, eval = FALSE}
...
In this vignette, we demonstrate the potential consequences of statistical analysis that ignores dependencies in data. Such overfitting can result in **misestimated effect sizes combined with underestimated variance leading to *p*-values that are too low**. This is also referred to as *pseudoreplication* see [@hurlbert1984pseudoreplication; @lazic2010problem; @eisner2021pseudoreplication] and a "unit of analysis error" [@hurlbert2009ancient]. 

This vignette is a simplified illustration and brief explanation of overfitting/pseudoreplication. It is not comprehensive. Critically, overfitting is not limited to analysis using simple regression/correlation and there are multiple types of pseudoreplication see  [@hurlbert1984pseudoreplication; @hurlbert2009ancient]. 
...
These two analyses produce distinct results. The overfit model falsely produces a precise moderate, negative correlation for higher MARS values being associated with lower times to capture targets (better performance). In contrast, the average model indicates a weak negative correlation with high uncertainty between dyads.
...
$$
Unlike the extent of the misestimation of the effect size and underestimation *p*-value -- without data, it may be possible to determine how much variability is suppressed by overfitting. The variability calculation requires two values: the actual sample size and the inflated sample size from the overfit model.
...
However, if this information is not reported it may still be possible to detect overfitting. For example, the actual sample size *N* and reported (ideally exact) *p*-value can be used to calculate their expected effect size. Then, the reported effect and expected effect can be compared.  
...
The prevalence of overfitting/pseudoreplication in published papers suggest it is a frequent statistical error across disciplines. For neuroscience, using a single issue of one journal, 36\% of papers had one or more results with suspected overfitting and 12\% of papers had definitive evidence of pseudoreplication [@lazic2010problem]. In human factors, using only results relevant to a systematic review and meta-analysis, overfitting was found in 28\% of papers (29 out of 103)
^[26 papers had all overfit results and 3 papers had some  results with overfitting. This calculation had 103 total papers = 74 (no overfitting) + 3 (partial overfitting) + 26 (all overfitting).][@bakdash2022SAmeta].